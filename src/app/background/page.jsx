"use client";

import React from "react";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

export default function Background() {
    const sectionRef = React.useRef(null);
    const educationRef = React.useRef(null);
    const experienceRef = React.useRef(null);

    const workExperience = [
        {
            role: "Solutions Engineer",
            company: "Telerivet Inc,Kathmandu,Nepal",
            period: "2024 – Present",
            description:
                "Developed and customized communication solutions using JavaScript on Telerivet’s cloud-based platform.",
        },
        {
            role: "Software Engineer",
            company: "BrainCast NeuroTechnologies, Kathmandu,Nepal",
            period: "2023 – 2024",
            description:
                "Contributed to projects involving the tracking and analysis of neuro-health data for research and diagnostic tools.",
        },
        {
            role: "Intern/Junior Software Engineer",
            company: "Javra Software,Kathmandu,Nepal",
            period: "2022 – 2023",
            description:
                "Gained practical experience in software engineering, agile methodologies, and real-world problem-solving.",
        },
    ];

    const education = [
        {
            degree: "Bachelor of Engineering in Electronics and Communication Engineering",
            institution: "Tribhuvan University, ERC Dharan",
            period: "Completed in 2022",
            description:
                "Specialized in Electronics and Communication Engineering, focusing on advanced research and practical applications.",
        },
        {
            degree: "Higher Secondary Education Board (HSEB)",
            institution: "Arniko Higher Secondary School, Biratnagar",
            period: "Completed in 2016",
            description:
                "Focused on PCBM (Physics, Chemistry, Biology, Mathematics), building a strong foundation for professional development.",
        },
        {
            degree: "School Leaving Certificate (SLC)",
            institution: "Bal Kalyan Vidhya Mandir (B.K.V.M), Biratnagar",
            period: "Completed in 2014",
            description:
                "Read from grade 1 to 10",
        },
    ];

    React.useEffect(() => {
        const ctx = gsap.context(() => {
            // Animate Education Items
            gsap.from(educationRef.current.children, {
                y: 50,
                opacity: 0,
                duration: 0.8,
                stagger: 0.2,
                scrollTrigger: {
                    trigger: educationRef.current,
                    start: "top 80%",
                    toggleActions: "play none none reverse",
                },
            });

            // Animate Experience Items
            gsap.from(experienceRef.current.children, {
                y: 50,
                opacity: 0,
                duration: 0.8,
                stagger: 0.2,
                scrollTrigger: {
                    trigger: experienceRef.current,
                    start: "top 80%",
                    toggleActions: "play none none reverse",
                },
            });
        }, sectionRef);

        return () => ctx.revert();
    }, []);

    return (
        <section ref={sectionRef} id="background" className="relative min-h-screen py-24 px-6 overflow-hidden">
            {/* Background Decoration */}
            <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-secondary/20 rounded-full blur-3xl -z-10 translate-x-1/3 -translate-y-1/4"></div>

            <div className="max-w-5xl mx-auto">
                {/* Page Title */}
                <div className="mb-20 text-center">
                    <h2 className="text-4xl font-bold mb-4">Background</h2>
                    <div className="w-16 h-1 bg-primary rounded-full mx-auto"></div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    {/* Education */}
                    <section>
                        <h3 className="text-2xl font-bold mb-10 flex items-center gap-3">
                            <span className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary">
                                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 14l9-5-9-5-9 5 9 5z" /><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" /></svg>
                            </span>
                            Education
                        </h3>
                        <div ref={educationRef} className="space-y-8 border-l-2 border-secondary/50 pl-8 ml-4 relative">
                            {education.map((edu, idx) => (
                                <div key={idx} className="relative group">
                                    <div className="absolute -left-[41px] top-1 w-5 h-5 rounded-full border-4 border-background bg-gray-400 group-hover:bg-primary transition-colors"></div>
                                    <div className="glass-card p-6 rounded-xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                                        <span className="inline-block px-3 py-1 rounded-full bg-secondary/50 text-xs font-semibold text-gray-600 mb-3">
                                            {edu.period}
                                        </span>
                                        <h4 className="text-xl font-bold text-foreground">{edu.degree}</h4>
                                        <p className="font-medium mb-3 text-primary">{edu.institution}</p>
                                        <p className="text-gray-600 leading-relaxed text-sm">{edu.description}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </section>

                    {/* Work Experience Timeline */}
                    <section>
                        <h3 className="text-2xl font-bold mb-10 flex items-center gap-3">
                            <span className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary">
                                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                            </span>
                            Work Experience
                        </h3>
                        <div ref={experienceRef} className="space-y-8 border-l-2 border-secondary/50 pl-8 ml-4 relative">
                            {workExperience.map((job, idx) => (
                                <div key={idx} className="relative group">
                                    {/* Dot */}
                                    <div className="absolute -left-[41px] top-1 w-5 h-5 rounded-full border-4 border-background bg-primary transition-transform group-hover:scale-125"></div>

                                    {/* Content */}
                                    <div className="glass-card p-6 rounded-xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                                        <span className="inline-block px-3 py-1 rounded-full bg-secondary/50 text-xs font-semibold text-gray-600 mb-3">
                                            {job.period}
                                        </span>
                                        <h4 className="text-xl font-bold text-foreground">{job.role}</h4>
                                        <p className="text-primary font-medium mb-3">{job.company}</p>
                                        <p className="text-gray-600 leading-relaxed text-sm">{job.description}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </section>
                </div>
            </div>

            <div className="mt-20 glass-card p-8 rounded-2xl max-w-3xl mx-auto text-center">
                <p className="text-gray-700 leading-relaxed italic text-sm md:text-md font-serif">
                    ॥ न हि ज्ञानेन सदृशं पवित्रमिह विद्यते ॥
                </p>
                <p className="text-sm text-gray-600 italic mt-2">
                    &quot;In this world, there is nothing so purifying as knowledge.&quot;
                </p>
                <p className="text-xs text-gray-500 mt-1">
                    — Bhagavad Gita 4.38
                </p>
            </div>
        </section >
    );
}
